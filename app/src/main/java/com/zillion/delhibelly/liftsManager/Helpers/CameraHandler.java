...
/*Not used, you can remove it and its calls in the Parent interface activites without any effect on the app*/
...
public class CameraHandler {
    ...
    private final String TAG = "Camera Handler";
    public CameraHandlerDone delegate;
    Context context;
    ...
    public CameraHandler(Context context, CameraHandlerDone delegate) {
    ...
    public Bitmap onResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == IMAGE_PIC_CODE) {
                Log.i(TAG, "Capturing URI from camera");
    ...
    private void doCropping(int code) {
        ...
        cropIntent.putExtra("outputX", 144);//256
        cropIntent.putExtra("outputY", 96);//256
    ...
    private void resultOnCroppingCancel(Intent data) {
    ...
    public interface CameraHandlerDone {
        void onCropped();
    }
